...
function handleOpt(){
  ...
  $.ajax({
    url: apiUrl + 'otp', // Replace with your API endpoint
    type: 'POST',
    contentType: 'application/json',
    data: JSON.stringify({
      otp: otp
        ...
        response.dataList.images.forEach((imageSrc, index) => {
          let rawHtml = `
            <div class="image-preview">
              <img src="${apiUrl+imageSrc+'?token='+jwtToken}" alt="Image ${index + 1}" />
            </div>
          `;
...
function addServicesDate(rows, totalCount, currentPage, itemsPerPage) {
  ...
  rows.forEach((row, index) => {
    ...
    if (Number(row.expiryInDays) < 0) {
      ...
      statusClass = 'due';
      dueicon = `<div onclick="showPopup(${row})" class="due-icon"> <img src="images/service_due_icon.svg" alt=""> </div>`;
  ...
  rows.forEach((row, index) => {
    ...
    if (row.DaysOverdue > 0 && row.DaysOverdue > 10) {
      ...
      if (row.DaysOverdue < -30) {
        statusName = `Overdue 30+ days`
      }
      else
        statusName = `Overdue ${Math.abs(row.DaysOverdue)} days`
      statusClass = 'status-overdue';
      statusIcon = `<div style="display: block;" onclick="showPopup('${row.propertyId}','${row.availableTradeId}','${row.comment}')" class="due-icon"> <img src="images/service_due_icon.svg" alt=""
      class="service-due-icon"> 
  </div>`
