...
function verifyJwt(token, isFromLogin) {
  ...
  userRole = jwt['http://schemas.microsoft.com/ws/2008/06/identity/claims/role'].toLowerCase();
...
function initializeSearchHandlers() {
  ...
  if (btnSearch) {
    btnSearch.addEventListener('click', typeof handleSearch === 'function' ? handleSearch : null);
  }
}
...
function setuserRole(userRole) {
  ...
  document.getElementById('usernameMobileView').innerHTML = userName;
  ...
  if (agentSidebar && traderSidebar && adminSidebar) {
    if (userRole === "agent") {
      ...
      agentOnlyElements.forEach((item) => (item.style.display = "grid"));
    ...
    console.log("One or more sidebar elements are not found in the DOM.");
  }
}
...
function handleLogin(event) {
  ...
  button.innerHTML = '<span id="spinner"></span>';
  ...
  // let valid =true
  // if(!email){
  //   emailerror.innerHTML = EmptyEmailError;
  //   valid=false
  // }
  // if(password){
  //   passwordError.innerHTML = EmptyPasswordError;
  //   valid =false
  // }
  // if(!valid){
  //   return
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        if (response.data && response.data.useremail) {
          emailerror.innerHTML = LoginEmailError;
        } else if (response.data && response.data.password) {
          passwordError.innerHTML = LoginPasswordError;
        }
      }
    ...
    complete: function () {
      ...
      // button.classList.remove('cenetrLoader')
    }
...
function handleOpt() {
  ...
  // Validate OTP length (assuming it's 6 digits)
  if (otp.length !== 6) {
    // alert('Please enter a valid 6-digit OTP.');
    document.getElementById('otpError').innerHTML = otpEmptyError
    return;
  }
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        document.getElementById('otpError').innerHTML = otpInvalidError
      }
...
function updateMyPropertydata(page = 1, itemsPerPage = 10) {
  ...
  $.ajax({
    url: `${apiUrl}PropertyBook?pageNumber=${page}&pageSize=${itemsPerPage}&search=${searchValue}`, // Replace with your API endpoint
    type: 'GET',
    contentType: 'application/json',
    headers: {
    ...
    error: function (xhr, status, error) {
      ...
      addMyPropertydata(dummy, 0, page, itemsPerPage);
    }
  });
}
...
function addMyPropertydata(rows, totalCount, currentPage, itemsPerPage) {
  ...
  if (totalCount == 0) {
    accordionRows.innerHTML = `<h2>no data available</h2>`;
    accordionRows.style.display = 'flex'
    accordionRows.classList.add('ceneterNoData')
...
async function handleAddPropertices(event) {
  ...
  if (!floors) {
    document.getElementById('floorsError').innerHTML = 'please enter a floors.';
    valid = false;
  }
  if (!rooms) {
    document.getElementById('roomError').innerHTML = 'please enter a rooms.';
    valid = false;
  }
  if (!living) {
    document.getElementById('livingError').innerHTML = 'please enter a livings.';
    valid = false;
  }
  if (!washrooms) {
    document.getElementById('washroomError').innerHTML = 'please enter a washroom.';
    valid = false;
  }
  ...
  // add loader
  const button1 = document.getElementById('SaveButtonOne')
  button1.innerHTML = '<span id="spinner"></span>';
  const button2 = document.getElementById('SaveButtonTwo')
  button2.innerHTML = '<span id="spinner"></span>';
  ...
  $.ajax({
    ...
    complete: function () {
      if (id) {
        ...
        button1.innerHTML = 'Create';
        button2.innerHTML = 'Create';
      }
      // button1.disabled = false
      // button2.classList.remove('cenetrLoader')
      // button1.disabled = false
      // button2.classList.remove('cenetrLoader')
    }
  });
...
//         let imagePreview = document.getElementById('previewGrid');
//         imagePreview.innerHTML = ''
//         response.dataList.images.forEach((imageSrc, index) => {
//           let rawHtml = `
//             <div class="upload-section-2__preview-item">
//               <img src="${apiUrl + imageSrc + '?token=' + jwtToken}" class="upload-section-2__preview-img" alt="Image ${index + 1}">
//               <div class="upload-section-2__overlay" style="visibility: visible;">
//                 <div class="upload-section-2__overlay-icons">
//                   <img src="images/eye_icon.svg" alt="View" class="upload-section-2__view-icon">
//                   <img src="images/delete_icon.svg" alt="Delete" class="upload-section-2__delete-icon">
//                 </div>
//               </div>
//             </div>
//           `;
//           imgPaths.push(imageSrc);
//           console.log('imageSrc', imageSrc)
//           imagePreview.innerHTML += rawHtml;
//         });
...
function updateMyJobs(page = 1, itemsPerPage = 10) {
  ...
  $.ajax({
    ...
    error: function (xhr, status, error) {
      ...
      console.error('Error:', error);
      if (userRole == 'admin')
        addAdminJobs(dummyAdminJobs, 0, page, itemsPerPage)
      else
        addAgentMyjobs(dumyjobs, 0, page, itemsPerPage);
...
function addAgentMyjobs(rows, totalCount, currentPage, itemsPerPage) {
  ...
  rows.forEach((row, index) => {
    ...
    else if (row.jobStatus == 'QuoteApproved') {
    ...
    <div class="property-card">
        <div class="property-card-header">
          <div class="property-name">
            ${row.propertyAddress}
            <div class="sub-property-name">${row.propertyName} </div>
          </div>
...
function updateServices(page = 1, itemsPerPage = 10) {
  ...
  $.ajax({
    url: `${apiUrl}AvailableTrade?pageNumber=${page}&pageSize=${itemsPerPage}&propertyBookID=${propid}`, // Replace with your API endpoint
    type: 'GET',
    contentType: 'application/json',
    headers: {
      "Authorization": `Bearer ${jwtToken}`
...
function showPopup(prop_id, list_available_trade_id, comments) {
  const data = {
    prop_id,
    comments,
  ...
  let popdiv = document.getElementById('popupOverlay');
  const dataString = JSON.stringify(data);
  popdiv.setAttribute('data-info', dataString);
...
function closePopup() {
  let popdiv = document.getElementById('popupOverlay');
  popdiv.removeAttribute('data-info');
...
function confirmAction() {
  // add loader 
  const button = document.getElementById('confirmJobs')
  button.innerHTML = '<span id="spinner"></span>';
  button.disabled = true


  const divElement = document.getElementById('popupOverlay');
  const dataString = divElement.getAttribute('data-info');
  ...
  $.ajax({
    ...
    data: JSON.stringify(
      {
        ...
        list_available_trade_id: [data.list_available_trade_id],
        requested_on: null
      }
    ),
    headers: {
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        if (currentUrl.includes('services.html')) {
          updateServices()
          // Your code for services.html
        } else if (currentUrl.includes('index.html')) {
          updateDashAgentJObs();
          updateDashAgentprop();
          // Your code for index.html
        } else {
...
function showSerDeletePOP(id) {
  document.getElementById('servicesDeleteID').value = id;
...
function addServicesDate(rows, totalCount, currentPage, itemsPerPage) {
  ...
  rows.forEach((row, index) => {
    ...
    let deleteIcon = ''
    let imgresponsive = ''
    let deleteresponsive = ''

    if (row.serviceStatus.toString() !== "abc") {
      if (Number(row.expiryStatus) > 0 && Number(row.expiryStatus) <= 10) {
        status = `Due in ${row.expiryStatus} days`;
        statusClass = 'due';
        dueicon = `<div  onclick="showPopup('${row.propId}','${row.availableTradeId}','${row.comments}')" class="due-icon"> <img src="images/service_due_icon.svg" alt=""> </div>`;

      } else if (Number(row.expiryStatus) < 0) {
        if (Number(row.expiryStatus) < -30)
          status = `Overdue 30+ days ago`;
        else
          status = `Overdue ${Math.abs(row.expiryStatus)} days ago`;
    ...
    if (row.serviceStatus == 'ThirdPartyCompleted') {
      imgFull = ''
      deleteIcon = `<img  onclick="showSerDeletePOP('${row.id}')" src="images/delete_icon.svg" alt="View">`
      imgresponsive = ``;
      deleteresponsive = ` <img onclick="showSerDeletePOP('${row.id}')" src="images/delete_icon.svg" alt="View" class="action-icon-responsive">`
    }
    ...
    <div class="dropdown-toggle" aria-expanded="false">
      <div class="accordion-toggle" aria-expanded="false">
        ...
        <div class="completed-date">${row.completedOn.split(' ')[0]}</div>
        <div class="service-status">
          <div>
...
function updateprofile(event) {
  ...
  let valid = true;
  if (!firstName) {
    document.getElementById('FNError').innerHTML = SignupFirstName
    valid = false
  }
  if (!lastName) {
    document.getElementById('LNError').innerHTML = SignupLastName
    valid = false
  }
  if (!phone) {
    document.getElementById('phoneError').innerHTML = SignupNumber
    valid = false
  }
  if (!address) {
    document.getElementById('addressError').innerHTML = 'please enter a adress.'
    valid = false
  }
  if (!dob) {
  ...
  if (!city) {
    document.getElementById('countryError').innerHTML = 'please select a city.'
    valid = false
  }
  if (!state) {
    document.getElementById('stateError').innerHTML = 'please select a state.'
  ...
  // add loader 
  const button1 = document.getElementById('saveProfile1')
  button1.innerHTML = '<span id="spinner"></span>';
  button1.disabled = true

  // add loader 
  const button2 = document.getElementById('saveProfile2')
  button2.innerHTML = '<span id="spinner"></span>';
...
function forgotPassword(event) {
  ...
  if (!confirmPassword) {
    document.getElementById('cfmPassError').innerHTML = 'please enter confirm password.'
    valid = false
  }
  if (newPassword != confirmPassword) {
    document.getElementById('cfmPassError').innerHTML = 'please enter same password'
    valid = false
  }
  if (!valid) {
    return
  ...
  // add loader 
  const button1 = document.getElementById('changepassword1')
  button1.innerHTML = '<span id="spinner"></span>';
  button1.disabled = true

  // add loader 
  const button2 = document.getElementById('changepassword2')
  button2.innerHTML = '<span id="spinner"></span>';
  button2.disabled = true
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          reject('Failed to get data'); // Reject the Promise if the response is not successful
        }
      },
      error: function (xhr, status, error) {
        console.error('Error:', error);
...
function setProperticesData() {
  ...
  var searchInput = items.querySelector("#searchInput");
  var dropdown = items.querySelector("#propertiesDropdown");

  function updateHrVisibility() {
    // Show <hr> if the dropdown is open or an item is selected
    if (items.style.display === "block" || selected.innerHTML !== dropdown.querySelector(".property-light")?.innerHTML) {
  ...
  dropdown.addEventListener("click", function (e) {
    // console.log("insie drop")
    document.getElementById('seletedProprtices').classList.remove('selected-border')
...
function filterDropdown(query) {
  const items = document.querySelectorAll("#propertiesDropdown .dropProperticeItem");
  items.forEach(item => {
    const propertyName = item.querySelector(".property-light").textContent.toLowerCase();
    const propertyAddress = item.querySelector(".property-bold").textContent.toLowerCase();
    if (propertyName.includes(query.toLowerCase()) || propertyAddress.includes(query.toLowerCase())) {
...
let DashcurrentPage = 1;  // Keep track of the current page
const DashitemsPerPage = 7;  // Number of items per page
let DashisLoading = false;  // To prevent multiple API calls
let totalCountjobs;
function updateDashAgentJObs(page = 1, itemsPerPage = 7) {

  // show loader
  if (page == 1) {
    document.getElementById('LoaderagentDashJobs').style.display = 'block'
    document.getElementById('agentDashJobs').style.display = 'none'

  }

  console.log("jwtToken", jwtToken);
  console.log("page is ", itemsPerPage)

  if (DashisLoading) return;  // Prevent multiple requests
  DashisLoading = true;  // Set loading state

  $.ajax({
    url: `${apiUrl}Trade?pageNumber=${page}&pageSize=${itemsPerPage}&isFromAgentDashboard=true`, // Replace with your API endpoint
    type: 'GET',
    contentType: 'application/json',
    headers: {
      "Authorization": `Bearer ${jwtToken}`
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        totalCountjobs = response.totalCount;
        DashcurrentPage++;
      } else {
        addDashAgentJObs([], 0, page, itemsPerPage);
      }
      DashisLoading = false;  // Reset loading state
    },
    error: function (xhr, status, error) {
...
function addDashAgentJObs(rows, totalCount, currentPage, itemsPerPage) {
  removeLdragntJbs()
  if (totalCount == 0) {
    document.getElementById('LoaderagentDashJobs').style.display = 'none'
    document.getElementById('DashAgentJobsSec').style.display = 'none'
    document.getElementById('AgentjoDefault').style.display = 'flex'
    return
  }
  ...
  if (currentPage == 1) {
    ...
    // hide loader
    document.getElementById('LoaderagentDashJobs').style.display = 'none'
    document.getElementById('agentDashJobs').style.display = 'block'
  ...
  rows.forEach((row, index) => {
    ...
    var dates = row.requestedOn ? `
                <div class="footer-date">
                <img class="footer-icon" src="./images/date_icon.svg" alt="Date Icon">
                <span>${row.scheduledAt.split(' ')[0]}</span>
                </div>
              `: '';
    var times = row.SlotType ? `
    ...
    <div class="agent-property-card-header" onclick="toggleAccordion(this)">
      <div class="property-name">
        <img  src="./images/arrow_down.svg" alt="Arrow Icon" class="arrow-icon-accordian-2 arrowDashAgent">
        ${row.propertyAddress}
      </div>
      <div class="service-status">
        <div class="service-name-job">Service Name : ${row.ServiceType}</div>
        <div class="status ${statusClass}">Status : ${row.jobStatus}</div>
      </div>
    </div>
    <div class="admin-property-card-body">
...
let DashcurrentPageprop = 1;  // Keep track of the current page
const DashitemsPerPageprop = 7;  // Number of items per page
let DashisLoadingprop = false;  // To prevent multiple API calls
let totalCountprop;
function updateDashAgentprop(page = 1, itemsPerPage = 7) {

  // show loader
  document.getElementById('LoaderagentDashProps').style.display = 'block'
  document.getElementById('agentDashProps').style.display = 'none'


  console.log("jwtToken", jwtToken);

  if (DashisLoadingprop) return;  // Prevent multiple requests
  DashisLoadingprop = true;  // Set loading state

  $.ajax({
    url: `${apiUrl}Trade/selectTradesForAgentDashboard?pageNumber=${page}&pageSize=${itemsPerPage}`, // Replace with your API endpoint
    type: 'GET',
    contentType: 'application/json',
    headers: {
      "Authorization": `Bearer ${jwtToken}`
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        totalCountprop = response.totalCount;
        DashcurrentPageprop++;
      } else {
...
function addDashAgentProp(rows, totalCount, currentPage, itemsPerPage) {
  removeLdragntProp()
  if (totalCount == 0) {
    document.getElementById('LoaderagentDashProps').style.display = 'none'
    document.getElementById('DashAgentPropSec').style.display = 'none'
    document.getElementById('agentPropDefault').style.display = 'flex'
    return
  }
  ...
  if (currentPage == 1) {
    ...
    // hide loader
    document.getElementById('LoaderagentDashProps').style.display = 'none'
    document.getElementById('agentDashProps').style.display = 'block'
  ...
  rows.forEach((row, index) => {
    ...
    if (row.DaysOverdue > 0 && row.DaysOverdue < 10) {

      statusName = `Due in ${Math.abs(row.DaysOverdue)} days`
      statusClass = 'status-pending';
      statusIcon = `<div style="display: block;" onclick="showPopup('${row.propertyId}','${row.availableTradeId}','${row.comment}')" class="due-icon"> <img src="images/service_due_icon.svg" alt=""
      class="service-due-icon"> 
  </div>`
    }
    else if (row.DaysOverdue < 0) {
      if (row.DaysOverdue < -30) {
        statusName = `Overdue 30+ days ago`
      }
      else
        statusName = `Overdue ${Math.abs(row.DaysOverdue)} days ago`
      statusClass = 'status-overdue';
      statusIcon = `<div style="display: block;" onclick="showPopup('${row.propertyId}','${row.availableTradeId}','${row.comment}')" class="due-icon"> <img src="images/service_due_icon.svg" alt=""
      class="service-due-icon"> 
  </div>`
    ...
    <div class="agent-property-card-header" onclick="toggleAccordion(this)">
      <div class="property-name">
        <img src="./images/arrow_down.svg" alt="Arrow Icon" class="arrow-icon-accordian-2 arrowDashAgent">
        ${row.PropertyBookAddressLine1}
      </div>
      <div class="service-status">
      <div class="service-name-job">Service Name : ${row.TradeType}</div>
        
        <div class="statusPropGrid">
            <div class="status ${statusClass}">Status : ${statusName}</div>
            ${statusIcon}
        </div>
